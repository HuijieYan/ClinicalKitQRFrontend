...
const InboxNewSharingComponent = ({ display }) => {
  ...
  const send = () => {
    ...
    const sendingEquipmentIds = decodeEquipmentSelection(
      equipmentStore.getState()
    );
    console.log(sendingEquipmentIds);
    const sendingUsergroups = decodeUsergroupSelection(
      usergroupStore.getState()
    );
    console.log(sendingUsergroups);
    const time = new Date().toUTCString();
    Uploader.sendSharings(
      getHospitalId(),
      getUserName(),
      sendingEquipmentIds,
      sendingUsergroups,
