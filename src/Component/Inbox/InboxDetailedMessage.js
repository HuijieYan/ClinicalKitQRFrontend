...
const InboxDetailedMessage = ({ option, index, vacant, display }) => {
  ...
  if (display) {
    return (
      <Box
        sx={{ width: "1", padding: "1%", overflow: "scroll", paddingTop: "2%" }}
      >
        ...
        <List>
          {equipments.map((equipment, index) => {
            return (
              ...
              >
                <ListItemButton
                  onClick={handleSelected(index, equipment.saved)}
                >
                  <ListItemIcon>
                    <Checkbox
                      checked={
                        selected.indexOf(index) !== -1 || equipment.saved
                      }
                      edge="start"
                      tabIndex={-1}
                      disableRipple
