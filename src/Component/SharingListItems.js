...
const SharingListItems = ({ data, generateNodes }) => {
  ...
  function renderItems(node) {
    ...
    const isChecked =
      (children.length === 0 && index !== -1) ||
      (children.length > 0 &&
        children.every(
          (childId) => Number(childId) < 0 || selected.indexOf(childId) !== -1
        ));
    /**
    ...
    const isIndeterminate =
      !isChecked &&
      children.some((childId) => selected.indexOf(childId) !== -1);
          ...
          <FormControlLabel
            ...
            control={
              <Checkbox
                ...
                onChange={(e) => {
                  handleSelected(node.value, isChecked);
                }}
              />
            }
          />
  ...
  return <>{data.length > 0 && tree !== null ? rendering() : null}</>;
};
