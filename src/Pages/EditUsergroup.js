...
const EditUsergroup = ({groupUsername, selectedHospitalId}) => {
    ...
    useEffect(()=>{
        ...
        if (level === 3) {
            GetData.getAllHospitalsByTrust(getTrustId()).then((data) => setHospitals(data));
            ...
            {hospitals.length > 1 &&
                <Form>
                    ...
                    <select
                        ...
                        {hospitals.map(hospital => <option key={hospital.hospitalId} value={hospital.hospitalId} label={hospital.hospitalName}/>)}
                    </select>
                </Form>
            }
