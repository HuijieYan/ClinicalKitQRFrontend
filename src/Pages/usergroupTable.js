...
function createData(accountName, accountUsername, role,hospital) {
    return {
        accountName,
        accountUsername,
        role,
...
function createDataThree(accountName, accountUsername,role) {
    return {
        accountName,
        accountUsername,
        role,
...
const UserGroupTable = () => {
    ...
    useEffect(()=>{
        ...
        if (level === 3){
            ...
            GetData.getAllGroupsByTrust(UserStatus.getTrustId()).then((data)=>{
                ...
                for (let i = 0;i<data.length;i++){
                    ...
                    rowsData.push(createData(usergroup[0],usergroup[1],usergroup[2],usergroup[3]));
                }
                setRows(rowsData);
            })
        }
        if (level === 2){
            GetData.getAllGroupsByHospital(UserStatus.getHospitalId()).then((data)=>{
                ...
                for (let i = 0;i<data.length;i++){
                    var usergroup = data[i];
                    rowsData.push(createDataThree(usergroup[0],usergroup[1],usergroup[2]));
                }
                setRows(rowsData);
            })
        }
    ...
    const handleDelete = (event,rows,setRows, selected,setSelected) =>{
        ...
        for (let i = 0;i<selected.length;i++){
            ...
            axios.delete("http://localhost:8080/usergroup/delete/hospitalId="+rowLs[i].hospital+" username="+rowLs[i].accountUsername);
    ...
    return (
        <Box sx={{ width: '100%' }}>
            <Paper sx={{ width: '100%', mb: 2 }}>
                <EnhancedTableToolbar handleDelete={handleDelete} setSelected={setSelected} setRows ={setRows} rows={rows} selected={selected} tableTitle={'UserGroups'} />
                <TableContainer>
                    <Table
                        sx={{ minWidth: 400 }}
                                ...
                                .map((row, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <TableCell align="center">{row.accountUsername}</TableCell>
